...
import { addCart, removeitem } from "../../helpers/index.js";
import { decreaseitem } from "../../helpers/index.js";
...
const CartItem = ({ item: { id, title, text, img, color, shadow, price, cartQuantity } }) => {
  ...
  const onRemoveItem = async () => {
    await removeitem(localStorage.getItem("id"), id);
    dispatch(setRemoveItemFromCart({ id, title, text, img, color, shadow, price, cartQuantity }))
  }

  const onIncreaseItemQTY = async () => {
    await addCart(localStorage.getItem("id"), id)
    dispatch(setIncreaseItemQTY({ id, title, text, img, color, shadow, price, cartQuantity }))
  }
  const onDecreaseItemQTY = async() => {
    await decreaseitem(localStorage.getItem("id"), id)
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="grid items-center gap-4">
            ...
            <div className="flex items-center justify-around w-full">
              <button type="button" onClick={onDecreaseItemQTY} className="bg-theme-cart rounded w-6 h-6 lg:w-5 lg:h-5 flex items-center justify-center  hover:text-blue-900 hover:bg-blue-900 active:scale-90">
                <MinusIcon className="w-5 h-5 lg:w-4 lg:h-4 text-white stroke-[2]  hover:text-blue-700 hover:bg-blue-200" />
              </button>
